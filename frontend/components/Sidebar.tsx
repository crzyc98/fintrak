...
const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab }) => {
  ...
  const loadAccounts = async () => {
    try {
      ...
      console.error('Failed to load accounts for sidebar:', err);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const navItems = [
    ...
    },
    {
      ...
      icon: () => (
        <svg className="w-5 h-5 text-gray-500" fill="currentColor" viewBox="0 0 20 20">
          <path d="M4 4a2 2 0 00-2 2v1h16V6a2 2 0 00-2-2H4zM18 9H2v5a2 2 0 002 2h12a2 2 0 002-2V9zM5 13a1 1 0 112 0 1 1 0 01-2 0zm7 0a1 1 0 112 0 1 1 0 01-2 0z" />
        </svg>
      ),
    },
    {
    ...
    <aside className="w-64 flex flex-col bg-[#050910] h-full shrink-0 border-r border-white/5">
      <div className="p-6 h-full flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto space-y-8 pr-2 custom-scrollbar">
          {isLoading ? (
            <div className="text-gray-600 text-xs text-center py-4">Loading accounts...</div>
          ) : accounts.length === 0 ? (
            <div className="text-gray-600 text-xs text-center py-4">No accounts yet</div>
          ) : (
              ...
              return (
                <div key={type}>
                  <button className="flex items-center text-[10px] font-black text-gray-600 uppercase tracking-widest mb-4 group w-full">
                    <svg
                      className="w-3 h-3 mr-1.5 text-gray-700 rotate-90"
                      fill="currentColor"
                      viewBox="0 0 20 20"
                    >
                  ...
                  <div className="space-y-3.5">
                    {typeAccounts.map((account) => (
                      <div
                        key={account.id}
                        className="flex items-center justify-between text-[13px] font-medium leading-none"
                      >
                        <div className="flex items-center text-gray-400 min-w-0 pr-4">
                          <div
                            className={`w-1.5 h-1.5 rounded-full ${
                              account.is_asset ? 'bg-green-500' : 'bg-orange-500'
                            } mr-2.5 shrink-0`}
                          />
